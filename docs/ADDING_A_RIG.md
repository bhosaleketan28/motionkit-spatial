...
4. Define declarative inspector controls and compatible presets with explicit property ownership.
5. Create rig-owned demo media with exactly the declared slot count.
6. Choose one family from `RIG_FAMILIES`, add concise slug tags, set production maturity, and write gallery copy.
...
9. Verify startup validation, safe fallback, partial and full media, switching in both directions, presets, transport, all ratios, backgrounds, WebM, PNG, session restoration, reduced motion, focus, and responsive layouts.
